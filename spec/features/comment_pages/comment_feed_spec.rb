...
describe 'Comment feed' do
  ...
  describe 'Issue page', js: true do
  ...
  describe 'Other resource page' do
    let(:params) do
      {
        comment: {
    ...
    context 'Card page' do
      let(:commentable) { create(:card) }

      it 'contains the fetch-comments element' do
        visit project_board_list_card_path(commentable.project, commentable.board, commentable.list, commentable)
        expect(page).to have_selector("[data-behavior='fetch fetch-comments'][data-path='#{comments_path(params)}']")
      end
    end

    context 'Evidence page' do
      let(:commentable) { create(:evidence) }

      it 'contains the fetch-comments element' do
        visit project_node_evidence_path(commentable.project, commentable.node, commentable)
        expect(page).to have_selector("[data-behavior='fetch fetch-comments'][data-path='#{comments_path(params)}']")
      end
    end

    context 'Note page' do
      let(:commentable) { create(:note) }

      it 'contains the fetch-comments element' do
        visit project_node_note_path(commentable.project, commentable.node, commentable)
        expect(page).to have_selector("[data-behavior='fetch fetch-comments'][data-path='#{comments_path(params)}']")
      end
    end
  end
