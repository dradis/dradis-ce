...
RSpec.describe RevisionCollapser, versioning: true do
  ...
  let(:whodunnit) { 'adama@dradisframework.com' }

  describe '.collapse' do
    subject(:collapse_revisions) { described_class.collapse(resource, whodunnit, RevisionTracking::REVISABLE_EVENTS[:autosave]) }
    ...
    describe 'persisting original state' do
      it 'carrys original state forward over autosaves' do
        ...
        described_class.collapse(resource, whodunnit)
    ...
    context 'if no resource is passed' do
      subject(:collapse_revisions) { described_class.collapse(nil, nil) }
